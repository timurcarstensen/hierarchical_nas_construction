...
def get_dataloaders(
    dataset, data_path, workers, seed: int = 777, eval_mode: bool = False
):
    if dataset == "cifar10":
        ...
        val_set = datasets.CIFAR10(
            root=data_path, train=True, transform=val_test_transform, download=True
        )
        ...
        while len(val_split) < 5000 and counter < num_train:
            ...
            counter += 1
        train_idx, val_idx = (
            [i for i in range(num_train) if i not in val_split],
            val_split,
        )
        ...
        test_loader = torch.utils.data.DataLoader(
            datasets.CIFAR10(
                root=data_path, train=False, transform=val_test_transform, download=True
            ),
            batch_size=128,
            shuffle=False,
            num_workers=workers,
...
class CIFAR10ActivationObjective(Objective):
    ...
    def __call__(
        self, working_directory, previous_working_directory, architecture, **hp
    ):
        ...
        if not self.eval_mode:
            try:
                ...
                out_dict = {"val_error": 1.0, "test_error": 1.0}
        else:
            out_dict = evaluate(
                model,
                train_loader,
        ...
        # prepare result_dict
        val_err = out_dict["val_error"] if not self.eval_mode else 1.0
        results = {
...
def dummy_test(config, is_trainable=True, is_const: bool = False):
    ...
    if is_trainable:
        ...
        first_act_before = deepcopy(
            next(p.data for p in model.act.parameters() if p.requires_grad).flatten()
        )
        second_act_before = deepcopy(
            next(
                p.data for p in model.layer1[0].act1.parameters() if p.requires_grad
            ).flatten()
        )
    ...
    if is_trainable and not is_const:
        first_act_after = next(
            p.data for p in model.act.parameters() if p.requires_grad
        ).flatten()
        second_act_after = next(
            p.data for p in model.layer1[0].act1.parameters() if p.requires_grad
        ).flatten()
