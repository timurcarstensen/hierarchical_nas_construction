...
class Structure:
  ...
  def __init__(self, genotype):
    ...
    for idx, node_info in enumerate(genotype):
      ...
      for node_in in node_info:
        ...
        assert len(node_in) == 2 and node_in[1] <= idx, f"invalid in-node : {node_in}"
  ...
  def str2structure(xstr):
    ...
    for _i, node_str in enumerate(nodestrs):
      ...
      for xinput in inputs: assert len(xinput.split("~")) == 2, f"invalid input length : {xinput}"
  ...
  def gen_all(search_space, num, return_ori):
    ...
    assert num >= 2, f"There should be at least two nodes in a neural cell instead of {num}"
