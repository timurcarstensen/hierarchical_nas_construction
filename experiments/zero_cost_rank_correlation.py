...
def find_measures(
    ...
    for k, v in measures_arr.items():
        measure_score = v if k in ("jacov", "epe_nas", "nwot", "zen") else sum_arr(v)
    return measure_score
...
class ZeroCost(Predictor):
    ...
    def query(self, graph, dataloader=None, info=None):  # pylint: disable=unused-argument
        loss_fn = self.loss_fn() if self.loss_fn is not None else None
...
def evaluate(zc_proxy, x, loader, extract_model: Callable | None = None):
    zc_proxy.pre_process()
    test_pred = []
    for x_ in x:
        model = extract_model(x_) if extract_model is not None else deepcopy(x_)

        if zc_proxy.method_type == "zen":
...
if __name__ == "__main__":
    ...
    results = {}
    # works: plain, epe_nas, grad_norm, snip, l2_norm, params, jacov, flops, nwot, zen,
    # synflow, grasp
    # does not work:
    # warnings: fisher (?)
